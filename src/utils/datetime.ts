...
export const handleDisableAfterTime = (current: Dayjs, startDate: Dayjs, hour: number, minute: number) => {
...
export const handleDisableBeforeTime = (current: Dayjs, startDate: Dayjs, hour: number, minute: number) => {
    if (!dayjs(current).isSame(startDate, "day")) return null;
